...
GlobalStateVector nominalMode(uint8_t numberOfCycles) {
    ...
    for (uint8_t i = 1; i < numberOfCycles; i++) {
        for (uint8_t j = 0; j < NumberOfTimeStepsPerCycle; j++) {
...
measurementsProduction(Vector3f sunPositionECI, Vector3f satellitePositionECI, float albedo,
    ...
    magneticBody = magneticBody + Parameters::R * randomVector;
...
PRData initialiseNominal() {
    ...
    refDataAndP.pMekfErrorMatrix = Parameters::P;
